...
The maintained page and styles live in `riftvm/riftvm` under `docs/index.html` and `docs/styles.css`. This repository contains the publishing copy: copy the page, rename `styles.css` to `style.css` and update its stylesheet link, copy `copy-command.css` and `copy-command.js`, and copy referenced assets into `assets/` when updating it. Keep `catalog/` intact.
